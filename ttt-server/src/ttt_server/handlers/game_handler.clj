...
(defn ->game-handler-body [game]
  (let [{:keys [current-token winner draw board]} game
        board  (or board (game/starting-board (:board-size game)))
        status (->status-message current-token winner draw)]
    [:html
     (core/head "Tic Tac Toe")
     [:body
      [:h1 "Tic Tac Toe"]
...
(defn parse-move [params]
  [(Integer/parseInt (:row params))
   (Integer/parseInt (:col params))])

(defn ->human-move [game params]
  (if params
    (game/update-or-ensure-game game (parse-move params))
    game))

(defn next-non-human-state [game]
  (let [ended? (board/end-game? (:board game) (:board-size game))
        human? (= ((:current-token game) game) :human)]
    (if (or ended? human?)
      game
      (-> game
...
(defn initialize-game-state [game {:keys [board-size x-type o-type first-player]}]
  (assoc game
    ...
    :board (game/starting-board board-size)
    :X x-type
    :O o-type
...
(defn ->start-game-config [params]
  {:board-size   (if (= "4x4" (get params :board-size)) :4x4 :3x3)
   :x-type       (keyword (:player-x-type params))
   :o-type       (keyword (:player-o-type params))
   :first-player (keyword (:first-player params))})
